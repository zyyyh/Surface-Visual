...
public:
    ...
    {
        if(v1==tri.v1 && v2==tri.v2 && v3==tri.v3)
            return true;
...
static bool EdgeIntersection(vertex& v1,vertex& v2,Triangle& tri,vertex& intersect_v)
{
...
static int TriangleIntersection(Triangle& tri1,Triangle& tri2,vertex& intersect_v)
{
    int res = 0;
...
static void SurfaceIntersection(std::vector<Triangle>& surface1,std::vector<Triangle>& surface2,
                                std::vector<Triangle>& vecIntersectTri,std::vector<vertex>& vecIntersPoints)
{
    ...
    //unique
    vecIntersectTri.erase(std::unique(vecIntersectTri.begin(),vecIntersectTri.end()),vecIntersectTri.end());
    vecIntersPoints.erase(std::unique(vecIntersPoints.begin(),vecIntersPoints.end()),vecIntersPoints.end());
    return;
}

static void SurfaceIntersection(std::vector<vertex>& surface1,std::vector<vertex>& surface2,
                                std::vector<vertex>& vecIntersectTri, std::vector<vertex>& vecIntersectPoints)
{
    ...
    SurfaceIntersection(surface_tri1,surface_tri2,intersect_tri,vecIntersectPoints);
...
{
   float farea = v1.x*v2.y+v2.x*v.y+v.x*v1.y-v1.x*v.y-v2.x*v1.y-v.x*v2.y;
   return farea*0.5;
}
...
bool IsVertexInTriangle(vertex& v,vertex& v1,vertex& v2,vertex& v3)
{
    ...
    if(fabs(farea1+farea2+farea3-farea)<0.001)
        return true;
...
void zbuffer(glm::mat4 transform,std::vector<vertex>& allvertex,std::vector<vertex>& intersecVertex,
             std::vector<vertex>& hideVertex)
{
        ...
        for(int r=r0;r<=r1;++r)
        {
            for(int c=c0;c<=c1;++c)
            {
                ...
                if(bInTri)
                {
                    ...
                    if(pZbuffer[r*width+c]>zz)
                        pZbuffer[r*width+c]=zz;
    ...
    for(int i=0;i<numIntersect;++i)
    {
        ...
        if(pZbuffer[r*width+c]<v.z)
            hideVertex.push_back(v);
    }
    free(pZbuffer);pZbuffer=NULL;
    return;
